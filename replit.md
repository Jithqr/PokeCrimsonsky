...
- **Sprite System**: `MonSprite` (App.tsx, module-level), `FallbackSprite` (TrainingZone.tsx), `BattleSprite` (BattleArena.tsx) all use `useState`+`useEffect` for stable fallback chains — fixes flicker caused by component remounting. When a back sprite is fully exhausted, the front sprite is shown horizontally flipped (`scaleX(-1)`) so it faces the enemy. Custom sprites map lives in `artifacts/hexamon/src/lib/sprites.ts`.
- **Dex renamed**: `PokeTalesDex.tsx` → `CrimsonSkyDex.tsx`; exported component is `CrimsonSkyDex`. Screen key `"poketalesDex"` kept for save-data compatibility.
