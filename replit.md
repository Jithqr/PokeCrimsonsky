...
- **Crimson Sky Dex (PokeTalesDex.tsx) now shows all forms**: 10-tab strip: ALL · BASE · ✨ SHINY · MEGA · G-MAX · ALOLAN · GALARIAN · HISUIAN · PALDEAN · ALT FORM. ALL tab shows all 1109 base Pokémon + all 306 forms in one combined grid (each form card colored by its category). BASE/SHINY show the 1109 base Pokémon (shiny sprites via Showdown's `ani-shiny/` URLs); form tabs show `POKEMON_FORMS` filtered by `FormCategory`. FormDetail has a ✨ toggle button to view shiny sprites for any form/mega. PP stat removed from all move displays. Sprite fallback chain: `ani/*.gif` → `gen5/*.png` → `home/*.png`.
- **306 alternate forms** in `pokemon-forms.ts` (IDs 10001–10683). Added: 6 official Megas (Sceptile, Swampert, Sableye, Gallade, Audino, Rayquaza), 19 Crimson Sky exclusive Megas (Raichu X/Y, Absol Z, Garchomp Z, Lucario Z, Chimecho, Staraptor, Golurk, Meowstic, Crabominable, Golisopod, Scovillain, Glimmora, Tatsugiri, Baxcalibur, Heatran, Darkrai, Magearna, Zeraora), 8 Alcremie flavor forms (Ruby/Matcha/Mint/Lemon/Salted Cream, Ruby/Caramel/Rainbow Swirl), 4 Hisuian evolutions (Overqwil, Sneasler, Ursaluna, Basculegion-Male).
- **PP system fully removed**: `battle-engine.ts` no longer initializes or populates `pp` fields in `fromAppMon`. Moves are unlimited in all battles and the dex no longer displays a PP stat column.
