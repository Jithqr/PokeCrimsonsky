...
- **Regional Form Sprites (MEGA folder)**: 32 custom animated GIF sprites downloaded from the MEGA folder covering Alolan (Sandshrew, Sandslash, Vulpix, Ninetales, Dugtrio, Persian, Geodude, Graveler, Golem, Exeggutor), Galarian (Meowth, Slowbro, Slowking, Zapdos, Weezing, Moltres), Hisuian (Growlithe, Arcanine, Voltorb, Electrode, Typhlosion, Qwilfish, Sneasel, Samurott, Lilligant, Zorua, Zoroark, Braviary, Goodra, Avalugg, Decidueye), and Paldean (Wooper, Tauros-Combat) forms. All saved to `artifacts/hexamon/public/sprites/custom/`. The CUSTOM_SPRITES map in App.tsx and PokeTalesDex.tsx was updated with all new entries.
- **Gen 9 Shiny Sprites**: Added 20 shiny Gen 9 sprites to CUSTOM_SPRITES (Wochien, Chienpao, Tinglu, Chi-Yu, Iron Valiant, Miraidon, Iron Leaves, Okidogi, Munkidori, Fezandipiti, Ogerpon, Iron Boulder, Iron Crown, Terapagos, Pecharunt, Iron Hands, Iron Jugulis, Iron Moth, Iron Thorns).
- **BattleArena Sprite Fix**: Replaced the old single-URL `SPRITE_FRONT`/`SPRITE_BACK` functions in BattleArena.tsx with a new `BattleSprite` component that uses a multi-step fallback chain: local custom sprite → PokéShowdown ani/ → gen5/static → base Pokémon. This ensures custom megas, Gen 9, and regional forms show correctly in battles.
- **Sprite Fallback Improvements**: MonSprite in App.tsx now falls back to the base Pokémon (stripping -alola/-galar/-hisui/-paldea suffixes) when a regional form sprite fails to load. Added `formSpriteBaseFallback` to PokeTalesDex.tsx for the same behaviour in the Dex.
