...
function BattleSprite({ sprite, back = false, isShiny = false, style }: { sprite: string; back?: boolean; isShiny?: boolean; style?: React.CSSProperties }) {
  ...
  // Preload front URLs so the flip appears instantly when ani-back fails.
  useEffect(() => {
    if (!back) return;
    frontUrls.slice(0, 2).forEach(url => { const img = new Image(); img.src = url; });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sprite, back]);
  const urls = back && !flipped ? backUrls : frontUrls;
...
.bx-me-platform {
  position: absolute; bottom: 96px; left: 14px; width: 210px; height: 28px;
...
.bx-me-sprite  {
  position: absolute; bottom: 110px; left: 8px; width: 180px; height: 180px;
  display:flex; align-items:flex-end; justify-content:center; z-index: 4;
  transform: translateZ(0); will-change: transform; backface-visibility: hidden;
}
